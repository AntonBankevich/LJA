...
//GetEdgeInfo(std::map<RRVertexType, ag::Vertex> &vertexes,
//            std::list<dbg::Edge> &edges, const RawEdgeInfo &raw_edge_info,
//            int k, bool unique) {
//    int cnt = 1;
...
//            vertexes.emplace(std::piecewise_construct, std::forward_as_tuple(st), std::forward_as_tuple(st, seq.Prefix(k), ag::VertexData::DBGData(st)));
//        if(vertexes.find(en) == vertexes.end())
//            vertexes.emplace(std::piecewise_construct, std::forward_as_tuple(en), std::forward_as_tuple(en, seq.Suffix(k), ag::VertexData::DBGData(en)));
//        dbg::Vertex &st_v = vertexes.at(st);
//        dbg::Vertex &en_v = vertexes.at(en);
//        edges.emplace_back(ag::Edge::id_type(st_v.getInnerId(), cnt++), st_v, en_v, seq.Subseq(k), ag::EdgeData());
//        edge_info.emplace_back(st, en, &edges.back(), unique);
//    }
//    return edge_info;
//}
//
