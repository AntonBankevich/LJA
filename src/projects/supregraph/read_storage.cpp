...
////                dir.pop_front();
////                if(dir.empty()) {
////                    inner_index[dir.getStart().getId()].emplace_back(dir.getStart(), dir.cutLeft(), dir.getStart().size() - dir.cutRight());
////                    dir.invalidate();
////                }
////            }
////        }
...
//                dir.pop_front();
//                if(dir.empty()) {
////                    addInnerSegment({dir.getStart(), dir.cutLeft(), dir.getStart().size() - dir.cutRight()});
////                    addInnerSegment({dir.getStart().rc(), dir.cutRight(), dir.getStart().size() - dir.cutLeft()});
//                    dir.invalidate();
//                }
//            } else {
//                Edge &pedge = *(it - 1);
...
//                size_t cut_left = it - 1 == dir.begin() ? dir.leftCut() : 0;
//                size_t cut_right = it + 1 == dir.end() ? dir.rightCut() : 0;
...
//        }
//    }
////    for(Vertex &vertex : resolution.newVertices()) {
////        EdgePair p = resolution.get(vertex);
////        inner_index[vertex.getId()] += inner_index[p.first->getStart().getId()].embed({vertex, 0, p.first->fullSize()});
////        inner_index[vertex.getId()] += inner_index[p.second->getFinish().getId()].embed({vertex, p.first->rc().truncSize(), vertex.size()});
////    }
...
////            addInnerSegment({new_vertex, left, right});
////        }
////    }
...
////    }
////    for(Vertex &vertex : spg.vertices()) {
////        vertex_dive[vertex.getId()] = 0;
////    }
////    for(auto &read_rec : storage) {
////        size_t len = read_rec.path.len();
////        for(ReadDirection dir : {read_rec.forward(), read_rec.backward()}) {
////            VERIFY(dir.getStart().size() > dir.cutLeft())
////            size_t start_pos = dir.cutLeft();
////            if(start_pos + len > dir.getStart().size())
////                vertex_dive[dir.getStart().rc().getId()] = std::max(vertex_dive[dir.getStart().rc().getId()],
////                                                                       dir.getStart().size() - dir.cutLeft());
////
////            for(Edge & edge : dir) {
////                if(start_pos <= edge.rc().truncSize())
////                    break;
////                start_pos -= edge.rc().truncSize();
////                if(start_pos + len > edge.getFinish().size())
////                    vertex_dive[edge.getFinish().rc().getId()] = std::max(vertex_dive[edge.getFinish().rc().getId()],
////                                                                          edge.getFinish().size() - start_pos);
////                else
////                    inner_index[edge.getFinish().rc().getId()] += Segment<Vertex>(edge.getFinish().rc(), start_pos, start_pos + len);
////            }
////        }
////    }
//    prepareIndex();
...
//        if (!rr.path.empty()) {
//            size_t len = rr.path.len();
//            for(ReadDirection dir : {rr.forward(), rr.backward()}) {
//                for (auto it = dir.begin(); it != dir.end(); ++it)
//                    read_index[it->getId()].emplace_back(dir, it);
//                VERIFY(dir.getStart().size() > dir.leftCut())
////                size_t start_pos = dir.cutLeft();
////                if(start_pos + len > dir.getStart().size())
////                    vertex_dive[dir.getStart().rc().getId()] = std::max(vertex_dive[dir.getStart().rc().getId()],
////                                                                           dir.getStart().size() - dir.cutLeft());
...
////            addInnerSegment({rr.path.getStart(), rr.path.cutLeft(), rr.path.getStart().size() - rr.path.cutRight()});
////            addInnerSegment({rr.path.getStart().rc(), rr.path.cutRight(), rr.path.getStart().size() - rr.path.cutLeft()});
//            rr.path.invalidate();
//        }
//    }
//    reads_ready = true;
//}
//
//void PathIndex::fireMergePath(const std::vector<EdgeId> &path, Vertex &new_vertex) {
//    unprepare();
//    embedding.remap(path, new_vertex);
////    size_t right = path.getStart().size();
////    size_t left = 0;
...
////        inner_index[new_vertex.getId()] += inner_index[edge.getFinish().getId()].embed(vertex_embedding);
////    }
//}
//
...
//    Edge &new_edge = v == path.getStart() ? v.front() : *v.incoming().begin();
//    size_t start = 0;
...
//                                                          start + path.getStart().size() - edge.getFinish().size(),
//                                                          sz - start};
...
//        return {};
//    if(path.size() == 0) {
//        Segment<Vertex> res(embed(path.getStart().getId(), path.leftCut(), path.rightCut()));
...
//        new_path += embed(eid);
//    }
//    size_t extra_cut = new_path.getStart().size() - embed(path.getStart().getId()).size();
//    new_path.setCutLeft(path.leftCut() + extra_cut + new_path.leftCut());
//    new_path.setCutRight(path.rightCut() + new_path.rightCut());
//    new_path.normalize();
