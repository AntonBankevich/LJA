...
namespace repeat_resolution {
    ...
    inline bool operator==(const RRVertexType & lhs, const RRVertexType & rhs) { return lhs.id == rhs.id; }
    inline bool operator!=(const RRVertexType & lhs, const RRVertexType & rhs) { return !operator==(lhs, rhs); }
    inline bool operator< (const RRVertexType & lhs, const RRVertexType & rhs) { return lhs.id < rhs.id; }
    inline bool operator> (const RRVertexType & lhs, const RRVertexType & rhs) { return  operator< (rhs, lhs); }
    inline bool operator<=(const RRVertexType & lhs, const RRVertexType & rhs) { return !operator> (lhs, rhs); }
    inline bool operator>=(const RRVertexType & lhs, const RRVertexType & rhs) { return !operator< (lhs, rhs); }

    std::ostream& operator<<(std::ostream & os, const RRVertexType & vertex_property) {
        size_t new_len = 7;
...
namespace repeat_resolution {
    struct RREdgeProperty {
        ...
        RREdgeProperty& operator+=(const RREdgeProperty& rhs) {
            dbg_edge_ids.insert(dbg_edge_ids.end(), rhs.dbg_edge_ids.cbegin(), rhs.dbg_edge_ids.cend());
            return *this;
        }
    };
    inline RREdgeProperty operator+(RREdgeProperty lhs, const RREdgeProperty &rhs) {
        lhs += rhs;
        return lhs;
    }
    ...
    class RRGraph :
        public graph_lite::Graph<
            ...
            /*Container neighbors_container_spec=*/graph_lite::Container::MULTISET> {
    ...
    public:
        ...
        void resolve_graph() {
            ...
            for (auto it = begin(); it != end(); ++it) {
                ...
                VERIFY(indegree != 1 or outdegree != 1); // a node cannot be on a non-branching path
                if (node_has_loop(node)) {
                    // TODO. Loops need special care. For now, skip
                }
                if (indegree == 0 or outdegree == 0) {
                    /* isolated vertex or a (multi-)tip should be skipped
                     * multi-tip (indegree = 0, outdegree > 1 or vice versa) technically can be resolved
                     * but that will not increase the length of contigs and thus is meaningless.
                     * It will also invalidate iterator unless we switch to ordered containers
                     */
                } else if (indegree == 1) {
