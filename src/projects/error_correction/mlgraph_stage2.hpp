...
static void analyseGenome(SparseDBG &dbg, KmerIndex &index, const std::string &ref_file,
...
MLGraph2EC(logging::Logger &logger, const std::experimental::filesystem::path &dir,
        ...
        size_t unique_threshold, bool diploid, bool debug, bool load, std::string reference) {
    ...
    dbg::TournamentPathCorrector tournamentPathCorrector(dbg, readStorage, threshold, reliable_coverage, diploid, 60000);
    ...
    auto cdbg = Component(dbg);
    std::ofstream dbg_edges(dir / ".." / "edges_reliable.csv");
    size_t remark = 0;
    for(dbg::Edge &edge : cdbg.edges()) {
        double coverage = edge.getCoverage();
        int size = edge.truncSize();
        dbg_edges << edge.getInnerId() << "," << coverage << "," << size << "," << int(edge.is_reliable) << std::endl;
    }
    dbg_edges.close();
...
class MLGraph2CorrectionStage : public Stage {
public:
    MLGraph2CorrectionStage() : Stage(AlgorithmParameters(
            {"k-mer-size=5001", "window=500", "reference=none", "coverage-threshold=3", "reliable-coverage=10", "unique-threshold=40000", "diploid", "load"},
...
protected:
    std::unordered_map<std::string, std::experimental::filesystem::path> innerRun(logging::Logger &logger, size_t threads,
        ...
        return MLGraph2EC(logger, dir, input.find("reads")->second, input.find("pseudo_reads")->second, input.find("paths")->second,
                          threads, k, w, threshold, reliable_coverage, unique_threshold, diploid, debug, load, reference);
    }
};
