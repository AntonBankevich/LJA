...
namespace dbg {
    inline void InvalidateBad(logging::Logger &logger, size_t threads, ag::AlignedReadStorage<DBGTraits> &reads, size_t min_read_size,
...
#pragma omp parallel for default(none) schedule(dynamic, 100) shared(cnt, reads, message, is_bad, min_read_size, std::cout)
        for (size_t i = 0; i < reads.size(); i++) {
            ag::AlignedRead<DBGTraits> &alignedRead = reads[i];
            if (!alignedRead.valid())
                continue;
            GraphPath &al = alignedRead.getPath();
            ag::PathPosition<DBGTraits> left = al.firstPosition();
            ag::PathPosition<DBGTraits> right = al.lastPosition();
            ...
            size_t len = 0;
            for (ag::PathPosition<DBGTraits> pp = left; pp != right; ++pp) {
            ...
            if (middle_bad || left == right) {
                ...
                ag::GraphPath<DBGTraits> sub = al.subPath(left, right);
    ...
    inline void InvalidateLowCovered(logging::Logger &logger, size_t threads, ag::AlignedReadStorage<DBGTraits> &reads, double threshold,
    ...
    inline void InvalidateSubreads(logging::Logger &logger, size_t threads, ag::AlignedReadStorage<DBGTraits> &storage, SparseDBG &graph) {
        ag::SuffixTracker<DBGTraits> tracker(storage, graph, 0, 1000000000);
        tracker.fillFromStorage(logger, threads);
        for (ag::AlignedRead<DBGTraits> &alignedRead: storage) {
            const ag::SuffixRecord<DBGTraits> &rec = tracker.getSuffixRecord(alignedRead.getPath().frontEdge());
