...
namespace dbg {
    ...
    BulgePathCorrector::resolveBulgePath(const dbg::ReadAlignmentStorage &reads, const BulgePath &path) const {
        ...
        for (size_t i = 0; i < path.size(); i++) {
            if (path.isBulge(i)) {
                if (s1 == s2) {
                    ...
                    const ag::VertexRecord<DBGTraits> &vr = reads.getRecord(path.getVertex(last));
    ...
    std::string BulgePathCorrector::correctRead(dbg::GraphPath &path) {
        ...
        for (size_t i = 0; i < path.size(); i++) {
            if (!cases.empty() && cases.back().read_to == i &&
                cases.back().path_to < paths[cases.back().path_ind].size() &&
