...
//    TODO: this is strange to have declaration of AssemblyGraph here. Need to make sure it does not break anything.
    ...
    class BaseEdge {
        ...
        friend class EdgeCodeListener<Traits>;
    public:
...
//        TODO: try to get rid of this
    ...
    public:
        ...
        BaseEdge(id_type id, Vertex &_start, Vertex &_end, Sequence _seq) :
            ...
            if(!this->seq.empty())
                first_nucl = this->seq.Subseq(0, 1);
            resetEdgeCode();
        }
...
//        BaseEdge() : id(), start(nullptr), finish(nullptr), seq() {}
        ...
        static std::function<EdgeId(Edge &)> IdTransformer() {
            return [](Edge &edge) {return edge.getId();};
        }
        ...
        void resetEdgeCode() {edge_code = first_nucl;}
...
//        rc end vertices are locked by this process or otherwise prevented from modification by other processes
        ...
        const Sequence &firstNucl() const {
            return first_nucl;
        }
...
//        try to access rc() method in the rc edge.
    ...
    struct EdgePosition {
        ...
        bool operator==(const EdgePosition<Traits> &other) const {return edge == other.edge && pos == other.pos;}
        bool operator!=(const EdgePosition<Traits> &other) const {return !(*this == other);}
        bool operator<(const EdgePosition<Traits> &other) const {return *edge < *other.edge || (edge == other.edge && pos < other.pos); }
        bool operator>(const EdgePosition<Traits> &other) const {return *edge > *other.edge || (edge == other.edge && pos > other.pos); }
    };

    template<class Traits>
    std::ostream &operator<<(std::ostream &os, const EdgePosition<Traits> &epos) {
        os << epos.edge->getId() << "(" << epos.pos << ")";
        return os;
    }
...
//}
    ...
    typename Traits::Edge &BaseVertex<Traits>::getOutgoing(unsigned char c) const {
        ...
        for (Edge &edge : outgoing_) {
            if (edge.corporeal && (edge.firstNucl().empty() || edge.firstNucl()[0] == c)) {
                cnt++;
            }
        }
        VERIFY(cnt <= 1);
	...
	std::cout << "Outgoing edge not found" << std::endl;
        ...
        for (const Edge &edge : outgoing_) {
            std::cout << edge.firstNucl() << std::endl;
        }
        VERIFY(false);
    ...
    template<class Traits>
    template<class I>
    typename Traits::Edge &BaseVertex<Traits>::getOutgoingByIterator(I iterator) const {
        size_t cnt = 0;
        for (Edge &edge : outgoing_) {
            if (edge.corporeal && (edge.firstNucl().empty() || edge.firstNucl()[0] == *iterator)) {
                cnt++;
            }
        }
        VERIFY(cnt <= 1);
